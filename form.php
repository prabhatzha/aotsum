...
$id = $name = $email  = $state_id = $city_id  = $marks_range_id = $course_id = $stream_id = $course_year = $course_fee = $discount_percent = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($id)) {
        ...
        $sql = "INSERT INTO course_applications (name, email, state_id, city_id, marks_range_id, course_id, stream_id, course_year, course_fee, discount_percent) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssisiiiss", $name, $email, $state_id, $city_id, $marks_range_id, $course_id, $stream_id, $course_year, $course_fee, $discount_percent);
    } else {
        // Update existing application
        $sql = "UPDATE course_applications SET name = ?, email = ?, state_id = ?, city_id = ?,  marks_range_id = ?, course_id = ?, stream_id = ?, course_year = ?, course_fee = ?, discount_percent = ? 
                WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssisiiissi", $name, $email, $state_id, $city_id, $marks_range_id, $course_id, $stream_id, $course_year, $course_fee, $discount_percent, $id);
    }
...
<div class="container mt-5">
    <h2 class="mb-4">Aotsom Selection Form</h2>
    <button class="btn btn-primary add-button" onclick="location.href='data_table.php';" style=" position: absolute;
            top: 20px;
            right: 20px;
            z-index: 1000;">
