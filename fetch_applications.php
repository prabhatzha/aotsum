...
$query = "SELECT ca.id, ca.name, ca.email,  s.name AS state_name, c.city AS city_name,  CONCAT(marks_range_from, '-', marks_range_to, '%') AS marks_range, co.name AS course_name, st.name AS stream_name, ca.course_year, ca.course_fee, ca.discount_percent, ca.created_at 
